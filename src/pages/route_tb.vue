      ...
      <template #item.actions="{ item }">
        <div class="d-flex ga-2 justify-end">
          ...
          <v-icon
            ...
            icon="mdi-delete"
            size="small"
            @click="removeClick(item.route_id)"
          />
        </div>
      </template>
...
<script setup>
  ...
  function removeClick (route_id) {
    confirm.value = true
    selectedId.value = route_id
  }
  async function remove () {
    try {
      await db.query('DELETE FROM route WHERE route_id = $1;', [selectedId.value])
      await loadData()
    } catch (error) {
      addMessage('error', error)
    } finally {
  ...
  async function save () {
    if (!formModel.value.title.trim()) {
      addMessage('warning', 'Название маршрута не может быть пустым')
      return
    }
